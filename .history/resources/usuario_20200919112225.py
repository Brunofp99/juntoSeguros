...
update.add_argument('senha', type=str, required=True, help="Teh field 'senha' cannot be left blank")
...
class UserUpdate(Resource):
    @classmethod
    def put(cls, id):
        dados = update.parse_args()

        user_encontrado = UserModel.find_user(id)

        senha_incriptada = self.incipta_senha(dados['senha'])

        dados['senha']

        if user_encontrado:
            user_encontrado.update_user_password(**dados)
